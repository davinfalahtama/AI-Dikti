...
def main():
    ...
    # elif option == 'OpenAI':
    #     st.session_state["model"] = ChatOpenAi(st.session_state.chat_history)  # Assuming ChatOpenAi is defined elsewhere
        ...
        if st.button("Submit & Process"):
            if pdf_docs:
                with st.spinner("Processing..."):
                    raw_text = st.session_state["model"].process_files(pdf_docs)
                    text_chunks = st.session_state["model"].split_text_into_chunks(raw_text)
    ...
    if st.session_state["session_id"] in st.session_state["chat_history"]:
        user_session = st.session_state["chat_history"].get(st.session_state["session_id"])
    ...
    if prompt := st.chat_input("Say something", disabled= st.session_state["chat_input"]):
        ...
        if pdf_docs:
            ai_msg =  st.session_state["model"].run_invoke(prompt,st.session_state["session_id"])
            st.session_state["chat_history"] = st.session_state["model"].store
            end_inference_time = time.time()
            inference_time = end_inference_time - start_inference_time
            with st.chat_message("assistant"):
                response = st.write_stream(st.session_state["model"].generate_response(ai_msg['answer']))
